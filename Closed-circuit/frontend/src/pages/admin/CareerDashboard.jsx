...
export default function CareerDashboard() {
  ...
  const loadApplications = useCallback(async (pageOverride) => {
    ...
    try {
      ...
      const data = await apiRequest(`/api/admin/careers?${params.toString()}`, { token });
      setRows(data.rows || []);
  ...
  const openApplication = async (row) => {
    ...
    if (!token) return;
    try {
      const data = await apiRequest(`/api/admin/careers/${row.id}`, { token });
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      const data = await apiRequest(`/api/admin/careers/${selected.id}/status`, {
        token,
        method: 'PATCH',
  ...
  return (
    <AdminShell title="Job Candidates" subtitle="Career application management">
      {statusToast && (
        <div className={`mb-4 flex items-center gap-2 rounded-lg border px-4 py-3 text-sm ${statusToast.type === 'success' ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-red-500/30 bg-red-500/10 text-red-300'}`}>
          {statusToast.type === 'success' ? <CheckCircle size={16} /> : <AlertCircle size={16} />}
          {statusToast.message}
        </div>
      )}

      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-white/10 bg-[#0f172a]/60 p-6 shadow-2xl backdrop-blur-xl">
        <form onSubmit={handleSearch} className="grid gap-4 md:grid-cols-2 xl:grid-cols-6 mb-6">
          <div className="xl:col-span-2 relative">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, email, mobile..." className={`${inputClasses} w-full pl-9`} />
          </div>
          ...
          <button type="submit" className="rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-600">Apply Filters</button>
        </form>
        ...
        <div className="overflow-x-auto rounded-xl border border-white/10">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-400">Loading applications...</td></tr>
              ) : rows.length === 0 ? (
                ...
                rows.map((row, index) => (
                  <tr key={row.id} className="border-t border-white/5 text-slate-300">
                    ...
                    <td className="px-4 py-3">
                      <button type="button" onClick={() => openApplication(row)} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-semibold text-indigo-300 hover:bg-white/5">
                        <Eye size={14} /> View
                      </button>
                    </td>
                  </tr>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-slate-400">Showing {rows.length} of {total} applications</p>
          <div className="flex items-center gap-2">
            <button type="button" disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-300 disabled:opacity-40"><ChevronLeft size={16} /> Prev</button>
            <span className="text-sm text-slate-400">Page {page} of {totalPages}</span>
            <button type="button" disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-300 disabled:opacity-40">Next <ChevronRight size={16} /></button>
          </div>
        </div>
      </motion.div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/70" onClick={() => { setSelected(null); setResumeOpen(false); }} />
          <div className="relative z-10 w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 bg-[#0f172a] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <button type="button" onClick={() => { setSelected(null); setResumeOpen(false); }} className="text-slate-400 hover:text-white"><X size={20} /></button>
            </div>

            <div className="grid gap-3 sm:grid-cols-2 text-sm text-slate-300 mb-4">
              {detailFields.map(([label, value]) => (
                <p key={label}><span className="text-slate-500 block text-xs mb-0.5">{label}</span>{value || '-'}</p>
              ))}
            </div>

            <div className="mb-4">
              <p className="text-slate-500 text-xs mb-1">Product Understanding</p>
              <pre className="whitespace-pre-wrap rounded-lg border border-white/10 bg-black/20 p-4 text-slate-200 max-h-40 overflow-y-auto">{selected.productUnderstanding || '-'}</pre>
            </div>

            {selected.resume_url && (
              <button type="button" onClick={() => setResumeOpen(true)} className="mb-4 inline-flex items-center gap-2 rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-4 py-2 text-sm font-semibold text-indigo-300 hover:bg-indigo-500/20">
                <FileText size={16} /> View Resume
              </button>
            )}

            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
              <label htmlFor="career-modal-status" className="block text-xs font-semibold text-slate-400 mb-2">Application Status</label>
              <select id="career-modal-status" value={modalStatus} onChange={(e) => setModalStatus(e.target.value)} disabled={savingStatus} className={`${inputClasses} w-full`}>
                {CAREER_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
              <button type="button" onClick={() => void handleSaveStatus()} disabled={savingStatus || modalStatus === (selected.status || 'New')} className="mt-3 w-full rounded-lg bg-indigo-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-600 disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-5xl rounded-2xl border border-white/10 bg-[#0f172a] p-4 shadow-2xl">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setResumeOpen(false)} className="text-slate-400 hover:text-white"><X size={20} /></button>
            </div>
            {selected.resume_url.toLowerCase().includes('.pdf') ? (
              ...
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-8 text-center">
                ...
                <a href={selected.resume_url} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-600">Open Resume</a>
              </div>
            )}
          </div>
        </div>
